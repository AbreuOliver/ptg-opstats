...
import {
	...
	const SLUG = 'non-physical-assaults';
	...
	function confirmNoIncidentsToReport() {
		for (const row of FLAT_ROWS) {
			values[row.id] = Array.from({ length: COLUMNS.length }, () => 0);
		}
		setFormDraftSnapshot(draftKey, values);
		persistDraftNow();
	}
	...
	>
		<table class="carbon-grid w-full border-separate border-spacing-0">
			...
			>
				<tr>
					<th
						class="sticky left-0 z-20 min-w-[320px] border-r border-[#7d7d7d] bg-[#111111] px-3 py-3 text-left font-semibold dark:border-zinc-700 dark:bg-zinc-900"
					>
						Category
					</th>
					{#each COLUMNS as col}
				...
				{#each SECTIONS as section, sectionIndex}
					...
					<tr class="border-y border-[#8b8b8b] bg-[#f0f0f0] dark:border-zinc-700 dark:bg-zinc-800">
						<td
							...
							class="overflow-hidden rounded-t-lg border border-[#8b8b8b] bg-[#f0f0f0] px-3 py-3 pl-6 text-[1.05rem] font-bold text-zinc-900 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100"
						>
							{section.title}
						</td>
					</tr>
